...
var root = path_1.join(__dirname, 'ui');
...
R.get(/^\/images$/, function (req, res) {
    fs.readdir(path_1.join(__dirname, 'uploads'), function (err, filenames) {
        if (err)
            return res.die(err);
        // stupid Mac OS X with your .DS_Store files
        filenames = filenames.filter(function (filename) { return !/^\./.test(filename); });
        async.map(filenames, function (filename, callback) {
            var filepath = path_1.join(__dirname, 'uploads', filename);
    ...
    */
    form.parse(req, function (err, fields, files) {
        ...
        async.map(files, function (file, callback) {
            ...
            var filepath = path_1.join(__dirname, 'uploads', file.name);
    ...
    fs.open(cache_filepath, 'r', function (err, fd) {
        if (err) {
            if (err.code == 'ENOENT') {
                ...
                loge_1.logger.debug('fs.createReadStream error: %s', err);
...
R.get(/^\/images\/([^?]+)(\?.+|$)/, function (req, res, m) {
    ...
    var upload_filepath = path_1.join(__dirname, 'uploads', m[1]);
    var cache_filepath = path_1.join(__dirname, 'cache', m[1] + m[2]);
    ensureOpenImage(upload_filepath, cache_filepath, urlObj.query, function (err, fd) {
        if (err)
            return res.die('Could not prepare image: ' + cache_filepath);
        // res.setHeader('Expires', );
        fs.fstat(fd, function (err, stats) {
            if (err)
                return res.die('Could not stat image: ' + cache_filepath);
...
R.head(/^\/images\/([^?]+)(\?.+|$)/, function (req, res, m) {
    ...
    var filepath = (options.quality || options.resize) ?
        path_1.join(__dirname, 'cache', m[1] + m[2]) : path_1.join(__dirname, 'uploads', m[1]);
    fs.stat(filepath, function (err, stats) {
        if (err) {
            res.statusCode = 404;
